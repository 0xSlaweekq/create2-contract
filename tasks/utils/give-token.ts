...
task('mint-usdc')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.chain === 'eth') {
      ...
      await uniswapV2Router.connect(signer).swapETHForExactTokens(usdcAmountOut, [WETH, USDC], taskArgs.user, Date.now() + 1000 * 60 * 10, {
        value: countTokens[0],
      ...
      await uniswapV2Router.connect(signer).swapETHForExactTokens(usdcAmountOut, [WBNB, USDC_BSC], taskArgs.user, Date.now() + 1000 * 60 * 10, {
        value: countTokens[0],
        maxFeePerGas: parseUnits('21000', 'gwei'),
        maxPriorityFeePerGas: parseUnits('21000', 'gwei'),
      })
...
task('mint-usdt')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.chain === 'eth') {
      ...
      await uniswapV2Router.connect(signer).swapETHForExactTokens(usdtAmountOut, [WETH, USDT], taskArgs.user, Date.now() + 1000 * 60 * 10, {
        value: countTokens[0],
        maxFeePerGas: parseUnits('21000', 'gwei'),
        maxPriorityFeePerGas: parseUnits('21000', 'gwei'),
      })
      ...
      await uniswapV2Router.connect(signer).swapETHForExactTokens(usdtAmountOut, [WBNB, USDT_BSC], taskArgs.user, Date.now() + 1000 * 60 * 10, {
        value: countTokens[0],
        maxFeePerGas: parseUnits('21000', 'gwei'),
        maxPriorityFeePerGas: parseUnits('21000', 'gwei'),
      })
