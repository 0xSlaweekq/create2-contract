...
describe('BSC stress test', function () {
  ...
  describe('Test #0 - Stress for withdrawAssetsFromClones', function () {
    ...
    it('Test #0.1 - ', async function () {
      ...
      const txn = await universalFactory.connect(owner).withdrawAssetsFromClones(proxyAddresses, transferAmounts, jane.address, tokenAddress)
      await txn.wait()

      const txn2 = await universalFactory.connect(owner).withdrawAssetsFromClones(proxyAddresses, depositAmounts, jane.address, ZeroAddress)
      await txn2.wait()
