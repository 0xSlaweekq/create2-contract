...
describe('BSC test', function () {
  ...
  before(async function () {
    ...
    await prepareContracts({ thisObject: this, chain: 56 })
  ...
  describe('Test #4 - ETH to Token Swap and Asset Withdrawal to Another Address', function () {
    ...
    const tokenList = [
      ...
      '0x3AF6F4fA0D451C170dDeE3c79bAa8DC37DF322C5',
    ]
    // Amount of each token to be transferred
    const tokenAmount = BigInt(10e18)

    before(async function () {
      const router = this.uniV2Router ? this.uniV2Router : this.pancakeswapRouter
      const wethAddress = await this.weth.getAddress()

      // Loop through each token in the `tokenList` to swap ETH for the token and transfer it to `upgradeableProxyAddress`.
      for (const token of tokenList) {
        // Swap ETH for the native token using Uniswap V2 Router
        const tx = await router.connect(alice).swapExactETHForTokens(10, [wethAddress, token], aliceAddress, Date.now() + 1000 * 60 * 10, {
          value: depositAmount,
        })
        const txn = await tx.wait()
        expect(txn?.status).to.be.equal(1)
        ...
        const tokenContract = ((await ethers.getContractFactory('MockToken')) as ContractFactory as MockToken__factory).attach(
          token,
        ) as MockToken
        tokenContracts.push(tokenContract)

        // Transfer a predefined amount of the token to the UpgradeableProxy contract
        const tx2 = await tokenContract.connect(alice).transfer(upgradeableProxyAddress, tokenAmount)
        const txn2 = await tx2.wait()
        expect(txn2?.status).to.be.equal(1)
      }
    })
    ...
    it('Test #4.1 - Swap ETH for Native Tokens and Withdraw to Jane', async function () {
      ...
      for (const t of tokenContracts) {
        ...
        const b = await t.balanceOf(upgradeableProxyAddress)
        expect(b).to.be.equal(tokenAmount)

        // Get the balance of the token for Jane before withdrawal
        const j = await t.balanceOf(janeAddress)

        // Withdraw the tokens from the clone to Jane's address
        const tx = await universalFactory.connect(owner).withdrawAssetsFromClones([upgradeableProxyAddress], [b], janeAddress, t.target)
        const txn = await tx.wait()
        expect(txn?.status).to.be.equal(1)

        // Get the balance of the token in the clone address after withdrawal
        const c = await t.balanceOf(upgradeableProxyAddress)
        expect(c).to.be.equal(0)

        // Get the balance of the token for Jane after withdrawal and verify the change
        const d = await t.balanceOf(janeAddress)
